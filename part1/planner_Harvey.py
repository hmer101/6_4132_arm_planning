...
class Planner():
    ...
    def solve(self, state_curr, h_max, relaxed=False):
        # Initialise solution
        state = self.s_0 # NEED TO CHANGE when calling for relaxed??? - see below for relaxed -> does this solve?
        h_curr = float('inf') # HOW USE H_MAX TO PRUNE/SET MAX DEPTH??????? - see below for relaxed -> does this solve?
